...
*Settled* draws nothing at all. Every rate is taken as the plan states it, and a rate that does not come to a whole animal carries its remainder forward until it does — a herd at 12.4 born alive farrows 12, then 12, then 13, then 12, then 13, and averages exactly 12.4. Services hold at the stated rate on the same arithmetic; so do the sexes. The seed then does nothing and the plan has one answer, so two plans can be set side by side and the difference between them is the thing you changed. The same sow place that could not be called on a rolled plan comes out at 0.17% every time. What a settled plan will not show you is a bad year.

Settled does not mean uniform. Litter mates still carry different thriftiness and gilts still come into heat across the window, because a batch that all reached sale weight on one day is not a lorry-load any farm sends — the spread is handed out from a fixed ladder rather than drawn, so it is the same every run.

**Losses are placed, not rolled for.** A stage owes what its mortality percentage comes to on the pigs entering it, and that debt is settled in whole animals. Twelve piglets at 10% owe 1.2 deaths: one is booked now and the 0.2 is carried, so the fifth litter through loses two and five litters lose six between them. Which pigs it falls on is read off the animal itself — the plan seed, the stage, its birth cohort and its tag — rather than drawn off a shared stream whose position depends on how many other things happened first, so the same pig is equally hardy in two plans that differ somewhere else entirely. Testing every pig against a hazard each morning is honest arithmetic and quite wrong at this scale: on a two-sow herd it swung realised pre-weaning mortality between 10.7% and 14.1% across seeds on a 12.5% input, which is larger than most of the decisions a plan exists to weigh up. Placing the losses holds it between 12.40% and 12.52%.
...
| `src/lib/sim/variation.ts` | The two ways a plan comes out: rolled from the seed, or taken exactly. |
| `src/lib/sim/mortality.ts` | What each stage owes in losses, which pigs it falls on, and when. |
